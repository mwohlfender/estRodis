...
functions {
  ...
  real estRodis_stan_scaled_beta_lpdf(real a, real b, real p, real q, real x) {
  ...
  int estRodis_stan_determine_scaling_factor(real x) {
  ...
  real estRodis_stan_likelihood_log(int[] clusters_size, int[] clusters_freq, real R, real k, real mutation_proba, real detection_proba) {
    ...
    int upper_limit_size_identical_sequence_clusters = max(estRodis_stan_determine_scaling_factor(2 / detection_proba) * max(clusters_size), max(clusters_size) + 500);
