...
#'
#' @details The data frame `nodes` has to contain at least the columns `node_key` (id of node)
#' and `generation` (generation of nodes).
#'
#' The data frame `edges` has to contain at least the columns `from` (index of infector node in `nodes`),
#' `to` (index of infectee node in `nodes`) and `generation` (generation of infectee node).
#'
...
#' nodes = nodes, edges = edges, max_generation = min(max(nodes$generation), 5))
estRodis_plot_transmission_chain <- function(nodes, edges, max_generation, colour_nodes_edges = "navyblue") {
  ...
  if (nrow(edges) >= 1) {
    ...
    edges <- edges |> dplyr::mutate(node_key = edges$from) |> dplyr::left_join(ggraph::create_layout(graph_transmission_tree, layout = "tree")  |> dplyr::select(c("node_key", "x", "y")),
                                                                         by = "node_key") |> dplyr::rename("x_from" = "x", "y_from" = "y") |> dplyr::select(-"node_key")

    # add x and y coordinate of nodes in column "to"
    edges <- edges |> dplyr::mutate(node_key = edges$to) |> dplyr::left_join(ggraph::create_layout(graph_transmission_tree, layout = "tree")  |> dplyr::select(c("node_key", "x", "y")),
